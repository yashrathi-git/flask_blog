...
def delete_bloat():
    if current_user.image_file == 'default.jpg':
        return 
    picture_path = os.path.join(app.root_path, 'static/profile_pic', current_user.image_file)
...
def update_account():
    ...
    profile_pic = url_for('static',filename=f'profile_pic/{current_user.image_file}')
...
def user_posts(username):
    ...
    posts = Post.query.filter_by(user_id=user.id).order_by(Post.date.desc()).paginate(per_page=5,page=page)
    return render_template('profile/profile_page.html',username=user.username
                            ,posts=posts,email = user.email,
                            profile_pic=url_for('static',filename=f'profile_pic/{user.image_file}'),
                            profile=True) 