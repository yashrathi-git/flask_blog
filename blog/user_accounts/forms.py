...
class LoginForm(FlaskForm):
    email = StringField('Email', validators=[DataRequired(message='Email is required field'), Email(message='Email not valid')])
...
class RegistrationForm(FlaskForm):
    ...
    pass_confirm = PasswordField('Confirm password', validators=[DataRequired(message='Confirm Password is required field')])
    remember_me = BooleanField(label="Remember me", default=True)
...
class ResetRequest(FlaskForm):
    ...
    def validate_email(self,email):
        ...
        if user is None:
            raise ValidationError('Email not yet registered. Please register email first.')
...
class OTPForm(FlaskForm):
    otp = StringField(label='OTP', validators=[DataRequired(), Length(min=6,max=6,message='Invalid OTP')])
    submit = SubmitField('Register')